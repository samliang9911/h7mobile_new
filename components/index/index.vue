...
const props = defineProps({
 ...
 const emit = defineEmits(['update-tab-count']);

const setTabCount = (index, count) => {
	emit('update-tab-count', { index, count });
};
				...
				if (res.data.code == 1000) {
					...
					if (naviId.value == 1) {
						if (res.data.data.Flow_AssignTask) {
							setTabCount(0, res.data.data.Flow_AssignTask.Total);
					...
					//已办
					else if (naviId.value == 2) {
						if (res.data.data.Flow_AssignTask) {
							if (pageNo.value === 1) dataList.value = res.data.data
								.Flow_AssignTask.Items //已办数据赋值到 视图数组
							else dataList.value = [...dataList.value, ...res.data.data
								.Flow_AssignTask.Items
							]
							if (res.data.data.Flow_AssignTask.Items.length < 10) canLoadMome
					...
					//待阅
						...
						if (res.data.data.Flow_ReadRecord) {
							setTabCount(2, res.data.data.Flow_ReadRecord.Total);
							if (pageNo.value === 1) dataList.value = res.data.data
...
.middle {
	...
	.middleBox {
		...
		.middleHead {
			...
			// }
			.personNameIcon {
				/* 1. 布局优化：使用 Flex 替代 text-align，确保内容绝对居中 */
				display: inline-flex;
				align-items: center;
				...
				/* 3. 现代感配色：低饱和度背景 + 稍微深一点的文字色 */
				background-color: rgba(102, 193, 255, 0.15);
				/* 半透明背景，更柔和 */
				color: #0066cc;
				/* 主题色文字，对比度更好 */

				/* 4. 规范圆角：80rpx宽度的元素，20rpx的圆角看起来更精致 */
				border-radius: 20rpx;
