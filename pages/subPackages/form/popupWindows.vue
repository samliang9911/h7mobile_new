                ...
                <view style="width: 100%;display: flex;flex-wrap: wrap;">
                  <view v-for="(item, indexCheckL) in  propsNodeSelPerson" :key="indexCheckL">
                    <up-checkbox @change="nodeFn(item, indexCheckL, 'checkbox')" v-model="item['checked']"
                      :name="item['nodeName']">
                      <text
                        :style="{ paddingRight: '15rpx', color: randomColor(item, indexCheckL) }">{{ item['nodeName'] }}</text>
                    </up-checkbox>
                  </view>
                </view>
              </up-checkbox-group>
            ...
            <view v-else>
              ...
              <view v-if="selectNode">
                <up-radio-group v-model="aaa" style="justify-content: end;">
                  <up-radio v-for="(item,index) in parseArray(flowAllData.ActivityInfo.find(e=>e.ActivityID==selectNode)?.Executors_FK)"
                  :key="item" :name="item" :label="parseArray(flowAllData.ActivityInfo.find(e=>e.ActivityID==selectNode)?.ExecutorNames)?.[index]">
                  </up-radio>
                </up-radio-group>
              </view>
              <!-- <up-radio-group>
                <view style="width: 100%;display: flex;flex-wrap: wrap;">
                  ...
                  {{ console.log(propsNodeSelPerson) }}
