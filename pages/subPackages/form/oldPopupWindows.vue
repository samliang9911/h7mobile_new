<template>
	<view style="position: relative;background-color: #22222200;">
		<up-popup safeAreaInsetBottom z-index="10076" :show="popupBoxShow" mode="bottom" round="15"
			:length="popupLength()" @close="close">
			<!-- <up-toast ref="uToast" z-index="10077" /> -->
			<view class="popupContent" style="height: 100%">
					...
					<view>
						<up-radio-group v-model="selectedValue">
							<up-radio :disabled="false" :activeColor="colorBackG" style="margin-left: 50rpx;"
								@change="showIdeaDecision(item)" v-for="(item, indexRadioY) in propsIdeaDecision"
								:key="indexRadioY"  :label="item['CodeName']" :name="item['CodeName']">
							</up-radio>
						</up-radio-group>
					</view>
				</view>
				...
				<!-- 流转节点 -->
				<view class="popupHead" v-if="propsNodeSelPerson['length'] > 0">
					<view style="width: 25%;">
						<text>流转节点:</text>
					</view>
					<view style="width: 75%;">
							...
							<up-checkbox-group>
								<view style="width: 100%;display: flex;flex-wrap: wrap;">
									<view v-for="(item, indexCheckL) in  propsNodeSelPerson" :key="indexCheckL">
										<up-checkbox @change="nodeFn(item,indexCheckL,'checkbox')"
											v-model="item['checked']" :name="item['nodeName']">
											<text :style="{paddingRight:'15rpx',color:randomColor(item,indexCheckL)}">{{item['nodeName']}}</text>
										</up-checkbox>
									</view>
								</view>
							</up-checkbox-group>
						...
						<view v-else>
							<up-radio-group>
								<view style="width: 100%;display: flex;flex-wrap: wrap;">
									<view v-for="(item, indexGroupD) in propsNodeSelPerson" :key="indexGroupD">
										<up-radio @change="nodeFn(item,indexGroupD,'radio')" :name="item['nodeName']">
											<text :style="{color:randomColor(item,indexGroupD)}">{{item['nodeName']}}</text>
										</up-radio>
									</view>
								</view>
							</up-radio-group>
						...
						<view v-for="(item, indexNewM) in propsNodeSelPerson" :key="indexNewM">
							<view v-if=" propsNodeSelPerson['length'] >= 1 && item['checked'] && item['TaskAssignMode'] === 'All'"
								...
								<up-cell-group style="width: 100%;">
									<view :style="'width: 100%;display: flex;align-items: center;'+'background-color: '+item['color']+'0a'+';'">
										<view style="flex: 3;flex-wrap: wrap;">
											<!-- <scroll-view style="width: 95%;" class="scroll-view_H" scroll-x="true" scroll-left="120"> -->
											<view style="display: flex;white-space: nowrap;flex-wrap: wrap;">
												<view style="padding-left: 20rpx;" 
												v-for="(item2, index2) in item['xuanrenArr']" :key="index2">
													<up-checkbox @change="choosePersonFn(item2,indexNewM,index2,'checkbox')" v-model="item2.checked" :name="item2['Executor']">
										...
										<view style="padding: 15rpx 0;flex: 1;display: flex;align-items: center;justify-content: center;">
											<view :style="'color:#eeeeee;text-align: center;width: 50%;border: 1rpx solid '+item['color']+'0a'+';padding: 10rpx 0;background-color: '+item['color']+';border-radius: 10rpx;'"
												@click="addTheFn(item)">
												<text>增加</text>
											</view>
										</view>
									...
									<view :style="'width: 100%;display: flex;align-items: center;'+'background-color: '+item['color']+'0a'+';'">
										<view style="flex: 3;flex-wrap: wrap;">
											<view style="display: flex;white-space: nowrap;flex-wrap: wrap;">
												<view style="padding-left: 20rpx;"
													v-for="(item2, index2) in item['xuanrenArr']" :key="index2">
													<up-radio @change="choosePersonFn(item2,indexONE,index2,'radio')" :disabled="false" :name="item2.Executor">
														<text :style="'color:'+ item['color']+';'">{{item2.Executor}}</text>
													</up-radio>
												</view>
											</view>
				...
				<view class="popupInput">
					<view class="popupInputContent">
						<textarea :maxlength="500" @input="inputIdeaContent" v-model="opinionContent" placeholder="请输入内容"></textarea>
        ...
        <up-toast ref="uToast" />
				<up-loading-page :loading="loadingPage.loading" :loadingText="loadingPage.text" 
          bgColor="rgb(255 255 255 / 45%)" color="#676767" fontSize="14"/>
				<!-- 悬浮按钮 -->
				...
				<!-- 保存等按钮 -->
				<view class="toAddBtn" :style="popupAddBtn ? propsContentList['length'] == 4 ? 'width:82%;': propsContentList['length'] == 2 ? 'width:41%;': propsContentList['length'] == 3 ?'width:72%;': propsContentList['length'] == 1 ? 'width:42%;': propsContentList['length'] <= 0 ? 'display:none;' : 'width:92%;':'opacity: 0;' ">
          <view @click="contentListBtn(item)" class="floatBtn" v-for="item in contentList" :key="item.id">
            <view class="floatBtnIcon">
              <up-icon :name="item.iconName" :color="item.active ?''+''+colorBackG+'AA'+''+'': ''+colorBackG+''" size="15"></up-icon>
            </view>
            <view class="floatBtnText" :style="propsContentList['length'] > 4 ?'font-size: 25rpx;':''+ popupAddBtn?'opacity: 1;transition: all .6s ease-in-out;':'opacity: 0;transition: all .2s ease-in-out;'">
              <text>{{item.text}}</text>
            </view>
          </view>
				</view>
				...
				<view v-for="item in 2" :key="item" class="pickBox" :style="pickShow ? '':'display:none;'">
					...
					<view :style="item === 1 ?'display:none;':'margin-left:5%;transform: scale(1, 0.5);width: 90%;font-size: 40rpx;border-bottom: 1px solid #e4e7ed;'"></view>
						...
						<view @tap="radioBtn(item)"
							...
							<svg t="1658297792493" class="icon" viewBox="0 0 1024 1024" version="1.1" p-id="10551"
								...
								<path
									...
									fill="#4395ff" p-id="10552" data-spm-anchor-id="a313x.7781069.0.i8"
									class="selected"></path>
								<path
									...
									fill="#515151" p-id="10553" data-spm-anchor-id="a313x.7781069.0.i11"
									class="selected"></path>
							</svg>
							<text
								style="padding-left: 10rpx;font-size: 25rpx;">{{item === 1 ?  remindText['title1'] : item === 2 ? remindText['title2']:"提醒"}}</text>
						...
						<scroll-view style="width: 74%;" scroll-x="true" scroll-left="120">
							<view style="color:#212529;display: flex;align-items: center;white-space: nowrap;">
								<view v-for="(it,count) in superInfo(item)" :key="count">
									...
									<text
										:style="'color:'+colorBackG+ ';white-space: nowrap;font-size: 25rpx;'">{{it.NAME}}</text>
								</view>
							</view>
						</scroll-view>
					</view>
		...
		<up-popup z-index="10077" border-radius="16" :show="show" mode="bottom" @close="show=false">
			<up-cell-group>
				<view :style="choiceUp ===3 ?'display:block':'display:none'" v-for="(item, index) in propsOpinion" :key="index">
					<up-cell
						:titleStyle="{'display':'flex','align-items':'center','justify-content':'center','width':'100%'}"
						:title="item['Opinion']"  @click="opinionFn(item)"></up-cell>
				</view>

				<view :style="choiceUp ===2 ?'display:block':'display:none' " v-for=" (item, index) in listSelect"
					:key="index" >
					<up-cell
						:titleStyle="{'display':'flex','align-items':'center','justify-content':'center','width':'100%'}"
				...
				<view :style="choiceUp ===1 ?'display:block':'display:none' " v-for=" (item, index) in listSelect"
					:key="index" >
					<up-cell
						:titleStyle="{'display':'flex','align-items':'center','justify-content':'center','width':'100%'}"
						:title="item['label']" @click="remind(item)"></up-cell>
		...
		<!-- <up-mask :show="mask" :custom-style="{backgroundColor:'#fffff52'}">
			<view class="hr-mask-box">
				<text class="hr-mask-box-text" v-show="maskText ? true : false">{{maskText}}</text>
			</view>
		</up-mask> -->
	</view>
</template>
...
<script setup>
	...
	const props = defineProps({
		...
		flowAllData: Object,
		opinion: Array,
		oldIdeaChose: [String,Number],
		oldIdeaCodeValue: String,
		oldIdeaCode: [Number,String],
		IsIdea: Boolean,
		getApproval: [Object,Array],
  ...
  const uToast = ref()
	...
	let operationTitle = ''; // 操作执行流转...按钮标题
	let operationModalShow = false; // 操作提示弹窗
	const propsContentList = ref([]);
	const propsNodeSelPerson = ref([]); // 流转节点
	const propsIdeaDecision = ref([]);
	let propsFlowAllData = ref({});
	const propsOpinion = ref([]);
	const backGround = ref(''); // 全局背景颜色
	const colorBackG = ref('');
	const supervise = ref([]); // 督办
	let Inform = []; // 知会
	let coordinateIndex = null; // 上次选人索引记录
	...
	let saveSubJson = {"field":{}}; // 保存提交的数据
	const opinionContent = ref(''); // 多行文本框内容
	let CodeID = ''; // 意见决策id
	let IdeaContent = ''; // 意见决策内容
	const baseUrl = '/api/flowGet/approvalData'; // 获取审核数据
	const executeUrl = '/api/flowAction/approvalHandle'; // 执行审核数据
	let nodeName = null; // 选择的节点名称
	const chooseNodeIndex = null; // 节点的索引
	let choosePersonIndex = null; // 节点的选人索引
	const choiceUp = ref(null); // 弹窗内容控制器
	const remindText = reactive({
	  title1: '提醒',
	  title2: '提醒'
	}); // 提醒按钮文字
	const mask = ref(false);
	let getApprovalHttpJson = {};
	const maskText = ref('');
	const emits = defineEmits(['popWinValue']); //传递父组件事件
		
		
	// 监听
	// watch(()=>props.oldIdeaChose, (newData) => {
	//   if (newData) {
	//     selectedValue.value = newData;
	//   }
	// });
	// 打开弹窗
	watch(()=>props.popupShow, (newData,oldData) => {
		if (newData) {
	...
	watch(()=>props.oldIdeaCodeValue, (newData) => {
	  ...
	  // saveSubJson.Idea = opinionContent.value;
	});
	...
	watch(()=>props.jierenArr, (newData,oldData) => {
	  ...
	  // console.log("propsNodeSelPerson", propsNodeSelPerson.value);
	});
	...
	watch(()=>props.flowAllData, (newData) => {
		...
		CodeID = propsFlowAllData.value.IdeaCode;
		saveSubJson.tag = "upd";
	...
	watch(()=>props.getApproval, (newData) => {
	  if (newData) {
		getApprovalHttpJson = newData;
	  }
	}, { deep: true });
	
	watch(()=>opinionContent.value, (newData) => {
	  if (newData) {
	    IdeaContent = newData;
	  }
	});
	...
	// 点击意见决策
	const showIdeaDecision = (item, name)=> {
		// 将意见决策添加到请求参数内容
		if (!name && item){
	...
	const nodeFn = (item, coordinate, chooseType)=> {
		// console.log("点击流转节点item",item)
		if (chooseType == 'radio') {
			item.checked = true;
			if (coordinateIndex != null && coordinateIndex != undefined)
				propsNodeSelPerson.value[coordinateIndex].checked = false;
	...
	// 悬浮按钮的子按钮的点击提示和实现功能2
	const operationFn = (item, switchUp) =>{
		if (switchUp === 0) {
			return;
		}
		floatBtnFn(item);
	...
	const contentListBtn = (item)=> {
		uni.showModal({
			title: '提示',
			content: operationTitle = "确定" + item.text + "吗?",
			success: (res) => {
				if (res.confirm) {
					operationFn(item, 1);
				} else if (res.cancel) {
		...
		operationModalShow = true;
	}
	
	// // 悬浮按钮的子按钮的点击提示和实现功能3
	const floatBtnFn = (item) =>{
		item.active = !item.active
		let btmFlag = item.ActionID;
		switch (btmFlag) {
			case 'BU_SAVE':
				// 保存
				saveSub(item);
			...
			case 'WF_HUIQIN':
				mask.value = true;
				maskText.value = "开始会签"
				publicSub("WF_HUIQIN", item);
				break;
			case 'WF_XIEBAN':
				// 协办
				mask.value = true;
				maskText.value = "开始协办"
				publicSub("WF_XIEBAN", item);
				break;
			case 'WF_SIGNTO':
				// 转签
				mask.value = true;
				maskText.value = "开始转签"
				publicSub("WF_SIGNTO", item);
				break;
			case 'WF_SIGN':
				// 加签
				mask.value = true;
				maskText.value = "开始加签"
				publicSub("WF_SIGN", item);
				break;
			case 'WF_READ':
				// 知会
				mask.value = true;
				maskText.value = "开始知会"
				publicSub("WF_READ", item);
				break;
		}
	}
	...
	const saveSub = (itemAction)=> {
		...
		params.push(saveSubJson);
		
		
		// 检测有没有修改的数据可提交，没有则终止后续操作
		const canSubmit = checkField(saveSubJson.field);
		if(!canSubmit){
			return;
		}
		...
		http_request(proxy, {params,...getApprovalHttpJson}, '', (res) => {
	...
	const finishTask = (itemAction) =>{
		let IdeaCode = 0;
		try {
			IdeaCode = Number(CodeID) ? CodeID : props.IsIdea || propsIdeaDecision.value.length >= 1 ?
				propsIdeaDecision.value[0].CodeID : CodeID;
		} catch (e) {
		...
		// 节点选人
		let toActivityInfo = []; //总体选人内容
		let activityInfoObject = {}; //toActivityInfo需要的数据
		let activityIDList = []; //临时存放的动作ID
		let activityNameList = []; //临时存放的节点名称
						...
						if (personItem.checked) {
							if (item.TaskAssignMode === 'All') //多选
							{
								if (toActivityInfo.length === 0) //0为首次
								{
									...
									activityInfoObject.activityID = item.ActivityID; //动作ID
									activityInfoObject.activityName = item.nodeName; //节点名称
									activityInfoObject.executorStrName = personItem.Executor; //所选人
									activityInfoObject.executorStrOID = personItem.ExecutorOID; //用户主键
									activityInfoObject.executorPostStrOID = personItem.ExecutorsPostOID; //岗位主键
									toActivityInfo.push(activityInfoObject);
								} 
								else {
		...
		jsonExecute.actionList.action = itemAction.ActionID;
    ...
    setTimeout(()=>{
      ...
      uToast.value.show({
		...
		http_request(proxy, jsonExecute, getUrlType(proxy) + executeUrl, (res) => {
			itemAction.active = !itemAction.active; //关闭执行高亮
	...
	const publicSub = (str, itemAction) =>{
		...
		// 赋值审批内容
		// if(!["WF_SIGNTO","WF_SIGN"].includes(str)){
			const canSubmit = checkField(saveSubJson.field,false);
			jsonSign.actionList.action = itemAction.ActionID;
			delete saveSubJson.field.Flow_AssignTaskOID;
			if(!canSubmit){	// ！！！提交的field为空时，从返回值(保留在数据库的值)进行赋值)
				jsonSign.actionList[0].IdeaCode = propsFlowAllData.value.IdeaCode || '';
				jsonSign.actionList[0].IdeaCodeValue = propsFlowAllData.value.IdeaCodeValue || '';
				jsonSign.actionList[0].IdeaContent = propsFlowAllData.value.IdeaContent || '';
			}else{
				jsonSign.actionList[0].IdeaCode = saveSubJson.field.IdeaCode || '';
				jsonSign.actionList[0].IdeaCodeValue = saveSubJson.field.IdeaCodeValue || '';
				jsonSign.actionList[0].IdeaContent = saveSubJson.field.IdeaContent || '';
			}
			// console.log("执行的saveSubJson:",saveSubJson)
		// }
		...
		uni.navigateTo({
			...
			events:{
				ChoicePerson(data){
					...
					http_request(proxy, jsonSign, getUrlType(proxy) + executeUrl, () => {
						mask.value = false;
						maskText.value = "";
						//清除监听，不清除会消耗资源
	...
	const remind = (TitleItem) =>{
		...
		{
			remindText.value.title1 = TitleItem.label;
		} else if (choiceUp.value == 2) //提醒2
		{
			remindText.value.title2 = TitleItem.label;
		}
		show.value = !show.value
	};
